...
document.addEventListener('DOMContentLoaded', () => {
    ...
    //User needs to be able to add a new item by clicking on the add button or by pressing 'enter'  add two event listeners, one on the button itself for a CLICK event and one that will listen to the KEYPRESS event so it will trigger on pressing 'enter' user clicks on the add button if there is text inside the item field then add it to the to do list
    addButton.addEventListener("click", function() {
        // grab the value of the input tag
        let newItem = document.getElementById("item").value;
        // if the input tag is not empty then run  function to add an item
        if (newItem) {
            // this function will add a new item to the todo list
            addItemTodo(newItem);
            // reset the input after adding a new item
    ...
    function addItemTodo(text) {
        ...
        // set the inside of our `li` the same as the parameter passed in function, which is going to be the value set by the user in the input field
        item.innerText = text;
